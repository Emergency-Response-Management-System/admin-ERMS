...
import { getHistoryInventory } from "../Redux/Actions/ImportStockAction";
...
const Header = () => {
  ...
  const historyInventory = useSelector((state) => state.historyInventory);
  useEffect(() => {
    dispatch(getHistoryInventory());
  }, []);
  ...
  useEffect(() => {
    ...
    window.document.title = dataconfigurationSingle?.nameWeb ? dataconfigurationSingle?.nameWeb : 'Admin Dashboard';
  
    // Tìm thẻ link với rel="apple-touch-icon"
    const appleTouchIconLink = window.document.querySelector("link[rel='icon']");
  
    // Cập nhật href của apple-touch-icon nếu có logo
    if (appleTouchIconLink) {
      appleTouchIconLink.href = dataconfigurationSingle?.logo ? dataconfigurationSingle?.logo : './images/user_avatar_default.png';
    }
  ...
  return (
    <>
      ...
      />
      <header style={{
        background: dataconfigurationSingle?.colorDefault ? dataconfigurationSingle?.colorDefault : '#007bff'
          ...
          <ul className="nav">
            ...
            <li className="dropdown nav-item me-4">
              <Link
                ...
                to="#"
              >
                <i className="fa fa-list-alt" aria-hidden="true" style={{ color: "white" }} />
                {numberUnread > 0 && (
                  <span className="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                    {numberUnread}
                  </span>
                  ...
                  <div className="notification-panel">
                    <div className="recent-activities">
                      <ul className="activity-list">
                        {
                          historyInventory?.historyInventory?.map((item, index) => {
                            return(
                              <li index={index} className="activity-item">
                              <div className="activity-line"></div>
                              <span className="activity-icon export-folder"></span>
                              <span className="activity-text">
                                <strong>{item?.userName}</strong> vừa { item?.status === 'Export' ? 'Xuất' : 'Nhập' } đơn hàng { item?.status === 'Import' && (<span>với giá trị <strong>{formatCurrency(item.total)}</strong></span>) }
                              </span>
                              <span className="activity-time">{  moment().diff(item?.createdAt, 'days')  === 0 ?  'Hôm nay' :  `${ moment().diff(item?.createdAt, 'days')} ngày trước`  }</span>
                            </li>
                            )
                          })
                        }
            ...
            <li className="dropdown nav-item">
              ...
              >
                <img
                  ...
                  src={ dataconfigurationSingle?.avatarDefault ? dataconfigurationSingle?.avatarDefault : '/images/user_avatar_default.png' }
                  alt="User"
                />
              </Link>
