...
//Report Dashboard
export const getReportProduct = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`/api/products/report-dashboard`, config);
    dispatch({ type: PRODUCT_REPORT_SUCCESS, payload: data });
  } catch (error) {
    const message =
      error.response && error.response.data.message
        ? error.response.data.message
...
//Report log
export const getLogReport = () => async (dispatch, getState) => {
  try {
    dispatch({ type: PRODUCT_REPORT_REQUEST });
    // userInfo -> userLogin -> getState(){globalState}
    const {
    ...
    const { data } = await axios.get(`/api/products/get-log-report`, config);
    dispatch({ type: PRODUCT_REPORT_SUCCESS, payload: data });
  } catch (error) {
    const message =
      error.response && error.response.data.message
